...
public class NoteListFragment extends Fragment {
    ...
    private com.android.notes.domain.NoteSource data;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(com.android.notes.R.layout.fragment_note_list, container, false);
    }
    ...
    private void initView(View view) {
        TextView userName = view.findViewById(com.android.notes.R.id.user_name);
        userName.setText(com.android.notes.domain.User.nameUser);
        recyclerView = view.findViewById(com.android.notes.R.id.recycler_view_notes);
        initRecyclerView();
    }


    private void initPopupMenu(View view) {
        Button btnPopupMenu = view.findViewById(com.android.notes.R.id.btn_popup_menu);
        btnPopupMenu.setOnClickListener(v -> {
            PopupMenu popup = new PopupMenu(context, v);
            popup.inflate(com.android.notes.R.menu.popup_main_menu);
            popup.show();
            popup.setOnMenuItemClickListener(item -> {
                if (item.getItemId() == com.android.notes.R.id.action_add) {
                    ...
                    com.android.notes.MainActivity activity = (com.android.notes.MainActivity) context;
                    com.android.notes.domain.Navigation navigation = activity.getNavigation();
                    dlgBuilder.show(navigation.getFragmentManager(), "NOTE_CREATE");
                    return true;
                } else if (item.getItemId() == com.android.notes.R.id.action_clear) {
                    data.clearNoteData();
    ...
    public interface Controller {
        void openNoteScreen(com.android.notes.domain.Note note, int position);
    }
    ...
    private void initRecyclerView() {
        ...
        adapter.setOnItemClickListener((view, position, itemId) -> {
            view.setBackgroundResource(com.android.notes.R.color.teal_700);
            if (itemId == adapter.CMD_UPDATE) {
                ...
                // и строку
                builder.setTitle(com.android.notes.R.string.alert_dialog_delete)
                        // Указываем сообщение в окне. Также есть вариант со строковым
                        // параметром
                        .setMessage(com.android.notes.R.string.exclamation)
                        // Из этого окна нельзя выйти кнопкой Back
                        .setCancelable(false)
                        // Устанавливаем отрицательную кнопку
                        .setNegativeButton(com.android.notes.R.string.no,
                        ...
                        // строкой
                        .setPositiveButton(com.android.notes.R.string.yes,
                                // Ставим слушатель, будем обрабатывать нажатие
                                (dialog, id) -> {
    ...
    public void addUpdateNote(com.android.notes.domain.Note newNote, int position) {
